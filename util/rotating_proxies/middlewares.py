...
class RotatingProxyMiddleware(object):
    ...
    * ``ROTATING_PROXY_BACKOFF_BASE`` - base backoff time, in seconds.
    ...
    def __init__(self,
        ...
        self.create_proxies(proxy_list)
        ...
        self.has_sent_close_spider = 0
        self.proxy_fetching_func = proxy_fetching_func

    def fetch_proxies(self):
        return self.proxy_fetching_func()

    def create_proxies(self, proxies=None):
        proxies = proxies or self.fetch_proxies()
        self.proxies = Proxies(self.cleanup_proxy_list(proxies), backoff=self.backoff)
        return self.proxies
    ...
    def from_crawler(cls, crawler):
        ...
        proxy_fetching_func_path = s.get('ROTATING_PROXY_FETCHING_FUNC')
        proxy_fetching_func = load_object(proxy_fetching_func_path)
        proxy_list = proxy_fetching_func()
        # comment this to make sure stop_if_no_proxies would work
        # if not proxy_list:
        #     raise NotConfigured()
        mw = cls(
            ...
            proxy_fetching_func=proxy_fetching_func,
            reanimate_interval=s.getint('ROTATING_REANIMATE_INTERVAL', 30 * 60),
        )
        mw.proxy_dispatcher = s.get('ROTATING_PROXY_DISPATCHER', '')
    ...
    def get_proxy_dispatcher(self):
        if isinstance(self.proxy_dispatcher, str):
            s_mod, s_attr = self.proxy_dispatcher.rsplit('.', 1)
            # print('-'*20, s_mod, s_attr)
            # mod = __import__(s_mod)
            mod = import_module(s_mod)
            attr = getattr(mod, s_attr)
            return attr
        else:
            raise NotImplementedError('proxy dispatcher must be string')
    ...
    def process_request(self, request, spider):
        ...
        if not self.proxy_dispatcher:
            proxy = self.proxies.get_random()
        else:
            proxy = self.get_proxy_dispatcher()(request, spider, self.proxies)

        if not proxy:
            if self.stop_if_no_proxies:
                if not self.has_sent_close_spider:
                    self.has_sent_close_spider = 1
                    spider.crawler.engine.close_spider(spider, 'stop_if_no_proxies')
                ...
                if proxy is None:
                    ...
                    if not self.has_sent_close_spider:
                        self.has_sent_close_spider = 1
                        spider.crawler.engine.close_spider(spider, 'no_proxies_after_reset')
                    raise CloseSpider("no_proxies_after_reset")

        logger.debug('set proxy: %s to %s', proxy, request.url)
...
class BanDetectionMiddleware(object):
    ...
    def _load_policy(cls, crawler):
        policy_path = crawler.settings.get(
            ...
            'util.rotating_proxies.policy.BanDetectionPolicy'
        )
