...
async def HttpRequest(url, body_data, img_data=""):
    rsp = Rsp()
    post_data = body_data
    files = {
    ...
    # files.update(post_data)
    # print(files)
    # rsp_data = await asks.post(url, data=files, headers=header)
    async with httpx.AsyncClient() as client:
        rsp_data = await client.post(url, data=post_data, files=files, headers=header, timeout=10)
...
class FateadmApi():
    ...
    #   rsp.err_msg： 异常时返回异常详情
    #
    async def QueryTTS(self, pred_type):
        tm = str(int(time.time()))
        sign = CalcSign(self.pd_id, self.pd_key, tm)
        param = {
            "user_id": self.pd_id,
    ...
    #   rsp.err_msg：异常时返回异常详情
    #
    async def Predict(self, pred_type, img_data, head_info="", src_url=""):
        tm = str(int(time.time()))
        sign = CalcSign(self.pd_id, self.pd_key, tm)
        param = {
            "user_id": self.pd_id,
        ...
        files = img_data
        rsp = await HttpRequest(url, param, files)
        if rsp.ret_code == 0:
    ...
    #
    async def Justice(self, request_id):
        if request_id == "":
            #
            return
        tm = str(int(time.time()))
    ...
    #   rsp.err_msg：异常时返回异常详情
    #
    async def Charge(self, cardid, cardkey):
        tm = str(int(time.time()))
        sign = CalcSign(self.pd_id, self.pd_key, tm)
