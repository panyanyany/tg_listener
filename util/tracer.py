...
def format_frame(frame_list, frame_id):
    return '-' * 20 + 'Thread {}'.format(frame_id).center(20) + '-' * 20 + '\n' + ''.join(frame_list)
 
def stacktraces3():
    tmp = tempfile.TemporaryFile()
    faulthandler.dump_traceback(tmp)
    tmp.seek(0)
    code = tmp.read()
    tmp.close()
